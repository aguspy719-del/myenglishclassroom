 ...
 * - icon-splash-1024.png         → the full logo at 1024px (same style as icon-512)
...
function overDark(img) {
  ...
  for (let i = 0, j = 0; i < img.px.length; i += 4, j += 3) {
    ...
    out[j] = Math.round(img.px[i] * a + DARK_BG[0] * (1 - a));
    out[j + 1] = Math.round(img.px[i + 1] * a + DARK_BG[1] * (1 - a));
    out[j + 2] = Math.round(img.px[i + 2] * a + DARK_BG[2] * (1 - a));
  }
  return out;
}
...
function maskable(src, size) {
  ...
  for (let y = 0; y < inner.h; y++) {
    const line = overDark({ w: inner.w, h: 1, px: inner.px.slice(y * inner.w * 4, (y + 1) * inner.w * 4) });
    line.copy(rgb, ((y + off) * size + off) * 3);
  }
  return encodePNG(size, size, rgb, 2);
}

// ── Splash icon: the full logo, identical in style to icon-512 ──
// Chrome's PWA splash screen uses the largest "any" manifest icon.
// Rendered full-bleed so the splash shows the complete tile logo.
...
// Maskable — dark full-bleed safe zone (Android requires opaque:
...
const appleLine = overDark(appleInner);
...
fs.writeFileSync("public/icons/icon-splash-1024.png", encodePNG(1024, 1024, resize(src, 1024, 1024).px, 6));
console.log("Generated: icon-192 (transparent), icon-512 (transparent), icon-splash-1024 (full logo), icon-maskable-192 (green bg, logo 90%), icon-maskable-512 (green bg, logo 90%), apple-touch-icon (green bg, logo 94%)");
