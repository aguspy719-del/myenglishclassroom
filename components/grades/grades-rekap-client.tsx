...
function StudentRekapTable({ classId, className }: { classId: string; className: string }) {
  ...
  const fetchGrades = useCallback(async () => {
    ...
    const [submissionsRes, attemptsRes] = await Promise.all([
      assignments.length > 0
        ? supabase.from("submissions").select("student_id, assignment_id, score").in("assignment_id", assignments.map((a) => a.id)).not("score", "is", null)
  ...
  // ── Class summary stats ─────────────────────────────────
  const finals = rows.map((r) => r.nilaiAkhir).filter((v) => v !== "-") as number[];
  ...
  const completed = rows.filter((r) => r.nilaiAkhir !== "-").length;
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <p className="text-xs text-gray-500 dark:text-gray-400">
          <span className="font-bold text-gray-900 dark:text-white">{completed}</span> dari {rows.length} siswa sudah punya nilai akhir
        </p>
