...
import {
  ...
  Clock, CheckCircle, AlertCircle, ArrowRight, Bell, Zap, Trophy,
} from "lucide-react";
          ...
          <div className="flex items-center justify-between mt-2">
            ...
            <div className="flex gap-1">
              {BADGES.slice(0, 4).map((badge) => (
                <span key={badge.id} className={`text-lg ${badges.includes(badge.id) ? "" : "opacity-30 grayscale"}`} title={badge.name}>
                  {badge.icon}
                </span>
              ))}
              {badges.length > 4 && <span className="text-xs text-blue-200 self-center">+{badges.length - 4}</span>}
            </div>
          </div>
        </CardContent>
      </Card>
